...
cdef class BatchedDataset:
    ...
    cpdef _worker(self, i):
        ...
        while self.current < self.idx.shape[0]:
            ...
            ws = torch.DoubleTensor(self.batch_size)
            memview = ix.numpy()
            memview_ws = ws.numpy()
            with nogil:
                count = self._getbatch(current, memview, memview_ws)
            if count < self.batch_size:
                ix = ix.narrow(0, 0, count)
                ws = ws.narrow(0, 0, count)
            self.queue.put((ix, ws, torch.zeros(ix.size(0)).long()))
        self.queue.put(i)
    ...
    cdef public long _getbatch(self, int i, long[:,:] ix, double[:] ws) nogil:
        '''
        ...
        Args:
            ...
            ix (long [:]) - A C memoryview of the result tensor that we will
                return to Python
            ws (double [:]) - A C memoryview of the result tensor that we will
                return to Python
